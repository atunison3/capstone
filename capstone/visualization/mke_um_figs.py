...
ax.set_title("Age distribution by turnout", fontsize=20, fontweight="bold",
             color=NAVY, pad=16)
...
leg = ax.legend(title="Turnout", fontsize=12, title_fontsize=13,
...
ax.text(0, -0.16, "respondents", ha="center", va="center",
...
for wdg, p in zip(wedges, pct):
    ...
    r = 0.80
    ax.text(r*np.cos(ang), r*np.sin(ang), f"{p:.0f}%",
            ha="center", va="center", fontsize=12, fontweight="bold",
            color="white" if wdg.get_facecolor()[0] < 0.5 else NAVY)

ax.set_title("Strength of party identification",
             fontsize=20, fontweight="bold", color=NAVY, pad=18)
legend_handles = [Patch(facecolor=c, edgecolor="white", label=l)
                  for c, l in zip(colors, labels)]
leg = ax.legend(handles=legend_handles, loc="center left",
                bbox_to_anchor=(0.98, 0.5), frameon=False, fontsize=12,
